...
(cond ((= total 0) (cons nil nil))
  ...
  ((= max-value 0) nil)
  (else (append
    (apply-to-all (lambda (s) (cons max-value s)) (list-partitions (- total max-value) (- max-pieces 1) max-value))
  ...
  (cond ((atom? expr) expr)
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))
               (params (cadr expr))
               (body   (cddr expr)))
           (cons (analyze form) (cons params (apply-to-all analyze body) ))))
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
            (analyze (cons (cons 'lambda (cons (car (zip values)) (apply-to-all analyze body))) (cadr (zip values))))))
        (else
